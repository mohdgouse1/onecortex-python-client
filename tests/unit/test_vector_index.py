...
VP = "/v1/vector"
IDX_NAME = "test-idx"
IDX_BASE = f"{BASE}{VP}/indexes/{IDX_NAME}"
...
def make_index():
    ...
    return pc.vector.index(IDX_NAME)
...
def test_scroll_basic():
    respx.post(f"{IDX_BASE}/vectors/scroll").mock(return_value=httpx.Response(200, json=SCROLL_RESPONSE))
    idx = make_index()
    result = idx.scroll(limit=2, include_values=True)
    assert len(result.vectors) == 2
...
def test_scroll_no_cursor_on_last_page():
    respx.post(f"{IDX_BASE}/vectors/scroll").mock(return_value=httpx.Response(200, json=SCROLL_LAST_PAGE_RESPONSE))
    idx = make_index()
    result = idx.scroll(cursor="cursor-xyz")
...
def test_sample_basic():
    respx.post(f"{IDX_BASE}/sample").mock(return_value=httpx.Response(200, json=SCROLL_LAST_PAGE_RESPONSE))
    idx = make_index()
    result = idx.sample(size=5)
    assert len(result.vectors) == 1
...
def test_query_batch_success():
    ...
    route = respx.post(f"{IDX_BASE}/query/batch").mock(return_value=httpx.Response(200, json=BATCH_RESPONSE))
    idx = make_index()
...
def test_query_batch_too_many_raises():
    ...
    with pytest.raises(ValueError, match="10"):
        idx.query_batch(queries=[{"vector": [1.0, 0.0, 0.0], "topK": 1}] * 11)
...
def test_query_with_score_threshold():
    ...
    route = respx.post(f"{IDX_BASE}/query").mock(return_value=httpx.Response(200, json=QUERY_RESPONSE))
    idx = make_index()
    result = idx.query(vector=[1.0, 0.0, 0.0], top_k=5, score_threshold=0.8)
    assert result.matches[0].id == "v1"
    body = json.loads(route.calls[0].request.content)
...
def test_query_with_group_by():
    ...
    route = respx.post(f"{IDX_BASE}/query").mock(return_value=httpx.Response(200, json=GROUP_RESPONSE))
    idx = make_index()
    group_by = {"field": "doc", "limit": 5, "groupSize": 2}
    result = idx.query(vector=[1.0, 0.0, 0.0], top_k=20, group_by=group_by)
...
def test_recommend_basic():
    ...
    route = respx.post(f"{IDX_BASE}/recommend").mock(return_value=httpx.Response(200, json=RECOMMEND_RESPONSE))
    idx = make_index()
    result = idx.recommend(positive_ids=["v1"], top_k=2)
...
def test_recommend_with_negatives():
    ...
    route = respx.post(f"{IDX_BASE}/recommend").mock(return_value=httpx.Response(200, json=RECOMMEND_RESPONSE))
    idx = make_index()
    idx.recommend(positive_ids=["v1"], negative_ids=["v9"], top_k=2, score_threshold=0.5)
    body = json.loads(route.calls[0].request.content)
